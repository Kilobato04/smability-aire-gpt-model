...
STATIC_TOPO_PATH = os.path.join(os.environ.get('LAMBDA_TASK_ROOT', '/var/task'), 'malla_valle_mexico_final.geojson')
STATIC_BUILD_PATH = os.path.join(os.environ.get('LAMBDA_TASK_ROOT', '/var/task'), 'capa_edificios_v2.json')
STATIC_ADMIN_PATH = os.path.join(os.environ.get('LAMBDA_TASK_ROOT', '/var/task'), 'grid_admin_info.json')

GRID_BASE_PATH = '/tmp/grid_base_prod_v36.csv'
...
def process_stations_data(stations_list):
    ...
    for s in stations_list:
        try:
            ...
            pm10_val = pm10_obj.get('avg_12h', {}).get('value') or pm10_obj.get('avg_1h', {}).get('value')
            pm25_obj = pollutants.get('pm25', {})
            pm25_val = pm25_obj.get('avg_12h', {}).get('value') or pm25_obj.get('avg_1h', {}).get('value')
    ...
    print(f"📊 Datos: {len(df)} st.", flush=True)
    return df
...
def prepare_grid_features(stations_df):
    ...
    try:
        ...
        print("🏗️ Construyendo Grid Físico + Admin...", flush=True)
        ...
        if os.path.exists(STATIC_TOPO_PATH):
            try:
                with open(STATIC_TOPO_PATH, 'r') as f:
                    data = json.load(f)
                    topo_df = pd.DataFrame([{'lat': f['geometry']['coordinates'][1], 'lon': f['geometry']['coordinates'][0], 'elevation': f['properties'].get('elevation', 2240)} for f in data['features']])
                    grid_df['altitude'] = inverse_distance_weighting(topo_df['lat'].values, topo_df['lon'].values, topo_df['elevation'].values, grid_df['lat'].values, grid_df['lon'].values).round(0)
            except: grid_df['altitude'] = 2240.0
        else: grid_df['altitude'] = 2240.0

        # 2. Edificios
        if os.path.exists(STATIC_BUILD_PATH):
            try:
                with open(STATIC_BUILD_PATH, 'r') as f:
                    build_df = pd.DataFrame(json.load(f))
                    grid_df['building_vol'] = inverse_distance_weighting(build_df['lat'].values, build_df['lon'].values, build_df['building_vol'].values, grid_df['lat'].values, grid_df['lon'].values).round(1)
            except: grid_df['building_vol'] = 0.0
        else: grid_df['building_vol'] = 0.0
        
        # 3. Admin Info (Alcaldías/Municipios)
        if os.path.exists(STATIC_ADMIN_PATH):
            try:
                print("🏛️ Cruzando capa administrativa...", flush=True)
                with open(STATIC_ADMIN_PATH, 'r') as f:
                    admin_df = pd.DataFrame(json.load(f))
                # Búsqueda vecinal rápida (Nearest Neighbor)
                tree = cKDTree(admin_df[['lat', 'lon']].values)
                dists, idxs = tree.query(grid_df[['lat', 'lon']].values, k=1)
                grid_df['mun'] = admin_df.iloc[idxs]['mun'].values
                grid_df['edo'] = admin_df.iloc[idxs]['edo'].values
            except: 
                grid_df['mun'] = 'N/A'
                grid_df['edo'] = 'N/A'
        else:
    ...
    if 'altitude' not in grid_df.columns: grid_df['altitude'] = interpolate_feature('alt', 2240.0).round(0)
...
def overwrite_with_real_data(grid_df, stations_df):
    ...
    for i, row in stations_df.iterrows():
        ...
        if dists[nearest_idx] < THRESHOLD:
            ...
            if not np.isnan(row['pm25_real']): grid_df.at[nearest_idx, 'pm25'] = row['pm25_real']
...
def predict_and_calibrate(model, grid_df, stations_df, real_col_name, output_col_name):
    FEATURES = ['lat', 'lon', 'altitude', 'building_vol', 'station_numeric', 'hour_sin', 'hour_cos', 'month_sin', 'month_cos', 'tmp', 'rh', 'wsp', 'wdr']
    ...
    if not calibration_set.empty:
        ...
        calibration_set['building_vol'] = 0 
        ...
        for i, row in calibration_set.reset_index().iterrows():
            st_name = row['name'][:18]; real = row[real_col_name]; base = station_preds[i]; bias = residuals[i]; final = base + bias
...
def calculate_ias_row(row):
    ...
    ias_pm25 = get_ias_score(row['pm25'], 'pm25')
    if ias_pm25 >= ias_o3 and ias_pm25 >= ias_pm10: return round(ias_pm25), 'PM2.5'
    elif ias_pm10 >= ias_o3: return round(ias_pm10), 'PM10'
    else: return round(ias_o3), 'O3'

def lambda_handler(event, context):
    print("🚀 Iniciando Lambda V36 (Admin Info + Buildings)...", flush=True)
    try:
        models = load_models()
        ...
        # INCLUIMOS MUNICIPIO, ESTADO y VOLUMEN EN LA SALIDA
        final_cols = ['timestamp', 'lat', 'lon', 'mun', 'edo', 'altitude', 'building_vol', 'tmp', 'rh', 'wsp', 'wdr', 'o3', 'pm10', 'pm25', 'ias', 'dominant', 'station', 'risk']
        ...
        return {'statusCode': 200, 'body': json.dumps('Grid V36 OK')}
    except Exception as e:
